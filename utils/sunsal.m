function [x,res_p,res_d,i] = sunsal(M,y,varargin)
...
elseif sum(lambda(:) ==  0)  && strcmp(addone,'yes')
    while (i <= AL_iters) && ((abs (res_p) > tol1) || (abs (res_d) > tol2)) % && norm(X_sol-z,'fro')/c_norm >conv_thre
        % save z to be used later
        if mod(i,10) == 1
            z0 = z;
        end
        ...
        i=i+1;
%         z_err_set=[z_err_set(2:end) norm(X_sol-z,'fro')]; %norm(X_sol-z,'fro')
