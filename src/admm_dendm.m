function M = admm_dendm(Y,A,M,dM,rho,eps_abs,eps_rel,mu,tau_incr,tau_decr,nIter,bool_rho,GGt,beta)
...
if bool_rho % update rho
    ...
    for q = 1:nIter
        ...
        % M update
        M = (YDAt + rho*(V - Lambda))/(AAt + rho*eye(R) + beta*GGt);
    ...
    [L1,U1] = lu(A*(A') + rho*eye(R) + beta*GGt);
    Binv = inv(U1)*inv(L1);
