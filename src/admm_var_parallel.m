function dM = admm_var_parallel(Y,A,M,dM,rho,gamma,eps_abs,eps_rel,mu,tau_incr,tau_decr,nIter,bool_rho)
...
if bool_rho % update rho
    parfor n = 1:N
        ...
        for q = 1:nIter
            ...
            % dMn update
            dM(:,:,n) = (C + rho_n*(V - Lambda))/(aat + (gamma + rho_n)*eye(R));
            
            % Splitting variable update
            V = bsxfun(@max, dM(:,:,n) + Lambda, -M);
